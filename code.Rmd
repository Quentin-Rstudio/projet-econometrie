...
graph_reg <- ggplot(data = base_complete, mapping = aes(x = sensi_env, y = part_depenses_env)) +
  theme_bw() +
  geom_point()+
...
Pour le modèle reg2, la significativité globale augmente (le  $R^{2}$ ajusté est plus élevé), ce qui est logique puisque l'on vient de ponctionner le modèle de variables non significatives (perc_env et soutien_pol_env)

```{r linear-regression}
>>>>>>> Stashed changes

# 1ère regression : juste sensi (2 définitions)
...
summary(reg1bis) # R2 adj : 0,26 + conn_env : p-value < 0.01
...
reg4 <- lm(data = base_complete, formula = depenses_brutes_env ~ sensi_env + GINI_2017 + EDUC_SUP_2017 + REV_MED_EURO_2017 + EPI.new)
...
reg8             <- lm(data = base_complete, formula = part_depenses_env ~ conn_env)

summary(reg8) # R2 ah : 0,29
